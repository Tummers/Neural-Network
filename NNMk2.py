...
import sys
"""
second neural network version.
weight initialisation with 1/sqrt(layer_size) std
cross entropy cost function 
regularising term in cost function
"""
class CostCrossEntropy:
    @staticmethod
    def function(activation, ideal):
        return np.sum(np.nan_to_num(-ideal * np.log(activation) - (1 - ideal) * np.log(1 - activation)))

    @staticmethod
    def delta(activation, ideal):
        return (activation - ideal)
    
class NN:
    def __init__(self, sizes, fname_train, fname_test, cost=CostCrossEntropy()):
        """
        ...
        #sqrt term in weight initialisation is 1 / sqrt of layer size, sharpens peak to increase init speed of learning
    ...
    def stochasticGradDescent(self, epochs, batch_size, eta, lmbda):
        """
    ...
    def updateNetwork(self, batch, eta, lmbda, n):
        ...
        self.weights = [(1 - eta*(lmbda/n))*w - (eta / len(batch)) * gw for w, gw in zip(self.weights, grad_ws)]
    ...
    def backProp(self, x, y):
        ...
        delta = self.cost.delta(activations[-1], y) #cross entropy delta
